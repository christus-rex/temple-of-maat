...
try {
  ...
  try {
    ...
    await page.waitForFunction(() => {
      ...
      return audio?.src?.endsWith('/assets/audio/maat-forty-two-declarations.web.opus') && audio.dataset.tm524StreamingFallback === 'web' && audio.readyState >= 1 && play && !play.disabled;
    }, null, { timeout: 45000 });
    ...
    await activate(page, 'Play');
    await page.waitForFunction(() => { const audio = document.querySelector('#tm524-chant audio'); return audio && !audio.paused; }, null, { timeout: 15000 });
    await wait(350);
    const playing = await page.evaluate(() => { const audio = document.querySelector('#tm524-chant audio'); return { paused: audio?.paused, currentTime: audio?.currentTime || 0 }; });

    await activate(page, 'Pause');
    await page.waitForFunction(() => document.querySelector('#tm524-chant audio')?.paused === true, null, { timeout: 15000 });
    ...
    await activate(page, 'Stop');
    await page.waitForFunction(() => document.querySelector('#tm524-chant audio')?.paused === true, null, { timeout: 15000 });
    const stopped = await page.evaluate(() => { const audio = document.querySelector('#tm524-chant audio'); return { paused: audio?.paused, currentTime: audio?.currentTime || 0 }; });
    ...
    const durationValid = live ? ready.durationFinite && ready.durationSeconds > 1012 && ready.durationSeconds < 1014 : ready.readyState >= 1;
    const successfulResponse = chantNetwork.some((entry) => entry.phase === 'response' && entry.status >= 200 && entry.status < 300);
    const assertions = {
      uiTriggerAssigned,
      ...
      trustedKeyboardPlayback: playing.paused === false && playing.currentTime >= 0,
      pauseWorks: paused === true,
      stopWorks: stopped.paused === true && stopped.currentTime < 0.15,
      networkResponse: successfulResponse,
    ...
    const result = { ok, base: base.href, failedAssertions, assertions, preEnsure, forcedEnsure, ready, playing, paused, stopped, chantNetwork, screenshotCaptured, pageErrors };
    fs.writeFileSync(path.join(outDir, live ? 'chant-streaming-v5.4-result.json' : 'result-v5.4.json'), JSON.stringify(result, null, 2));
